...
function buildHoyoActionRow() {
  ...
  return new ActionRowBuilder().addComponents(button1, button2, button3);
}

function buildHoyoEmbed(color) {
...
async function sendAlarmToChannel(channel, type) {
  const maxRetries = 3;
  let attempts = 0;
  let success = false;
  ...
  while (attempts < maxRetries && !success) {
    try {
      ...
      if (type === "hoyo") {
        const row = buildHoyoActionRow();
        const embed = buildHoyoEmbed(color);
        await channel.send({ embeds: [embed], components: [row] });
      } else {
        const embed = buildWuwaEmbed(color);
        await channel.send({ embeds: [embed] });
      }
      success = true;
      console.log(
        `Alarm sent successfully! (retry = ${attempts}, clientId = ${channel.id})`,
      );
    } catch (error) {
      attempts++;
      console.log(
...
async function triggerAlarm(client, channelIds, type) {
  ...
  for (const id of channelIds) {
    ...
    await sendAlarmToChannel(channel, type);
  }
}
